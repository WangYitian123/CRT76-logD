...
def main(smiles_list,args, exp_config, test_set):
    ...
    checkpoint = torch.load("my_model/CRT76-logD/model_pretrain_76.pth",map_location=torch.device('cpu'))#my_model/CRT76-logD/model_pretrain_76.pth"
...
if __name__ == '__main__':
    ...
    with open('my_model/CRT76-logD/args.pickle', 'rb') as file:
  #  with open('my_model/AFP76-logD/args.pickle', 'rb') as file:
        args =pickle.load(file)
    with open('my_model/CRT76-logD/configure.json', 'r') as f:
  #  with open('my_model/AFP76-logD/configure.json', 'r') as f:
        exp_config = json.load(f)
    args['device'] = torch.device('cpu')
    args = setup(args)
    test_set = pd.read_csv('example.csv',header=None, names=['smiles'])
 #   test_set = pd.read_csv('tautomer_test.csv',header=None, names=['smiles'])
