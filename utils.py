...
def collate(data):
    ...
    bg = dgl.batch(graphs)#把一个batch的图整合成一张图
...
def predict(args, model, bg):
    ...
    if args['edge_featurizer'] is None:
        ...
        # pka1_atom_list[~np.isinf(pka1_atom_list)]=1
        pka1_atom_list[np.isinf(pka1_atom_list)]=15
        pka2_atom_list=np.array(pka2_atom_list)
        # # pka2_atom_list[~np.isinf(pka2_atom_list)]=1
        pka2_atom_list[np.isinf(pka2_atom_list)]=0           
        # print("pka2_atom_list",pka2_atom_list)
        pka1_feature = torch.Tensor(pka1_atom_list/10).to(args['device'])
        pka2_feature = torch.Tensor(pka2_atom_list/10).to(args['device'])
        # add pka feature
        pka1_feature=pka1_feature.unsqueeze(-1)
        pka2_feature=pka2_feature.unsqueeze(-1)
        # print("pka1",pka1_feature)
        # print("pka2",pka2_feature)
        # pka1_feature=tool.fit_transform(pka1_feature.cpu().numpy())
        # pka2_feature=tool.fit_transform(pka2_feature.cpu().numpy())
        # pka1_feature=torch.tensor(pka1_feature).to(args['device'])
        # pka2_feature=torch.tensor(pka2_feature).to(args['device'])
        # print("shape: ",node_feats.shape)
        node_feats = torch.cat([node_feats,pka1_feature,pka2_feature],dim = 1)
        # print("shape: ",node_feats.shape)
   #     node_feats = torch.cat([node_feats,atom_value],dim = 1)
